...
int main(void)
{
	...
	{
		int chosen = rand() % len;
		char *name = arts[chosen]->name;
		int price = searchPrice(arts, name);
		if (price == -1)
		{
			printf("error: the absence of art is itself art\n");
			printf("name %s\n", name);
			return (1);
		}
		else
	...
	printf("invalid input test: name - asfdasdf, return - %d\n", searchPrice(arts, "asfdasdf"));
	return (0);
}
