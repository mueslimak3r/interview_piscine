...
int main(void)
{
	...
	{
		chosen = rand() % n;
		name = arts[chosen]->name;
		price = arts[chosen]->price;
		printf("random art - name: |%s| realvalue: |%d| found: |%d|\n", name, price, searchPrice(arts, n, name));
	}
	*/
	printf("n: %d\n", n);
	price = arts[0]->price;
	name = arts[0]->name;
	printf("first: |%s| realvalue: |%d| found: |%d|\n", name, price, searchPrice(arts, n, name));
	price = arts[n - 2]->price;
	name = arts[n - 2]->name;
	printf("last: |%s| realvalue: |%d| found: |%d|\n", name, price, searchPrice(arts, n, name));
	printf("invalid art - name: |%s| found: |%d|\n", "sdfasdfaefs", searchPrice(arts, n, "sdfasdfaefs"));
	return (0);
}
